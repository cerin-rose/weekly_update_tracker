...
function DirectoryRow({ student, update, onView }: { student: Student; update?: WeeklyUpdate; onView: () => void }) {
  return <div className="directory-review-row" role="row"><button className="student-name-button" onClick={onView}><span className="initials-avatar small">{student.initials}</span><span><strong>{student.name}</strong><small>{student.leadershipRole}</small></span></button><span className="directory-role" role="cell">{student.programAffiliation}<small>{student.primaryWorkstream}</small></span><p role="cell">{student.currentFocus}</p>{update && <StatusBadge status={getDisplayStatus(update)} />}<button className="text-button" onClick={onView}>View contribution history <ArrowRight size={14} /></button></div>;
}
...
export function Dashboard() {
  ...
  function handleTabKey(event: KeyboardEvent<HTMLButtonElement>, index: number) {
    ...
    setActiveTab((["response", "updates", "missing", "students"] as HomeTab[])[nextIndex]);
  ...
  return <main className="page-frame dashboard-page">
     <div className="page-intro dashboard-intro"><p className="eyebrow">Weekly review</p><p className="dashboard-greeting">Good morning, Dr. Lina</p><h1>Wednesday Meeting Review</h1><p className="page-description">Review contributions, questions, and support needs for the selected meeting.</p></div>

     <div className="review-toolbar"><div className="meeting-navigation"><button className="toolbar-icon-button" aria-label="Previous meeting" disabled={meetingDates.indexOf(meetingDate) === meetingDates.length - 1} onClick={() => moveMeeting(1)}><ArrowLeft size={15} /></button><label htmlFor="meeting-date"><span>Meeting</span><select id="meeting-date" value={meetingDate} onChange={(event) => setMeetingDate(event.target.value)}>{meetingDates.map((date) => <option key={date} value={date}>{formatDate(date)}</option>)}<option value="all">All meetings</option></select></label><button className="toolbar-icon-button" aria-label="Next meeting" disabled={meetingDate === meetingDates[0]} onClick={() => moveMeeting(-1)}><ArrowRight size={15} /></button></div><label htmlFor="team-filter"><span>Team</span><select id="team-filter" value={team} onChange={(event) => setTeam(event.target.value as TeamFilter)}>{teamOptions.map((option) => <option key={option}>{option}</option>)}</select></label><label htmlFor="workstream-filter"><span>Workstream</span><select id="workstream-filter" value={workstream} onChange={(event) => setWorkstream(event.target.value as WorkstreamFilter)}>{workstreamOptions.map((option) => <option key={option}>{option}</option>)}</select></label></div>
     ...
     <div className="review-summary" aria-label="Meeting review summary"><div><strong>{submittedCount} of {scopedStudents.length} submitted</strong></div><div><strong>{needsResponse.length}</strong><span>Needs response</span></div><div><strong>{missingStudents.length}</strong><span>Missing updates</span></div></div>

    <div className="hub-tabs" role="tablist" aria-label="Meeting review views">{[{ id: "response" as const, label: "Needs response" }, { id: "updates" as const, label: "All updates" }, { id: "missing" as const, label: "Missing updates" }, { id: "students" as const, label: "Student directory" }].map((tab, index) => <button key={tab.id} ref={(element) => { tabRefs.current[index] = element; }} id={`tab-${tab.id}`} className={activeTab === tab.id ? "active" : ""} role="tab" aria-selected={activeTab === tab.id} aria-controls={`panel-${tab.id}`} tabIndex={activeTab === tab.id ? 0 : -1} onClick={() => setActiveTab(tab.id)} onKeyDown={(event) => handleTabKey(event, index)}>{tab.label}{tab.id === "response" && <span className="tab-count">{needsResponse.length}</span>}</button>)}</div>
    ...
    <section id="panel-missing" className="hub-panel review-panel" role="tabpanel" aria-labelledby="tab-missing" hidden={activeTab !== "missing"} tabIndex={0}>{meetingDate === "all" ? <p className="empty-state">Choose a Wednesday meeting to see missing updates.</p> : <div className="missing-table" role="table" aria-label="Missing updates"><div className="missing-table-header" role="row"><span>Student</span><span>Role / Team</span><span>Last submission</span><span>Status</span></div>{missingStudents.length ? missingStudents.map((student) => <MissingRow key={student.id} student={student} lastSubmission={updates.filter((update) => update.studentId === student.id && update.meetingDate < meetingDate).sort((a, b) => b.meetingDate.localeCompare(a.meetingDate))[0]} />) : <p className="empty-state">Everyone in this view submitted an update.</p>}</div>}</section>
     ...
     <section id="student-directory" className="hub-panel review-panel" role="tabpanel" aria-labelledby="tab-students" hidden={activeTab !== "students"}><div className="directory-controls"><span>Student directory</span><label><span className="sr-only">Filter by role</span><select value={role} onChange={(event) => setRole(event.target.value)}>{roles.map((option) => <option key={option}>{option}</option>)}</select></label></div><div className="directory-table" role="table" aria-label="Student directory"><div className="directory-table-header" role="row"><span>Student</span><span>Team / workstream</span><span>Current focus</span><span>Status</span><span>Action</span></div>{scopedStudents.length ? scopedStudents.map((student) => { const latest = updates.filter((update) => update.studentId === student.id).sort((a, b) => b.meetingDate.localeCompare(a.meetingDate))[0]; return <DirectoryRow key={student.id} student={student} update={latest} onView={() => router.push(`/students/${student.id}`)} />; }) : <p className="empty-state">No students match these filters.</p>}</div></section>
