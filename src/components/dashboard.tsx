...
export function Dashboard() {
  ...
  return <main className="page-frame dashboard-page">
    ...
    {showDirectory ? <section className="directory-section" aria-labelledby="students-heading">
      ...
      <div className="directory-table-wrap">{loading ? <p className="loading-state">Loading students...</p> : directoryStudents.length ? <table className="review-table directory-table"><caption className="sr-only">Student roster</caption><thead><tr><th scope="col">Student</th><th scope="col">Program</th><th scope="col">Role</th><th scope="col">Workstream</th><th scope="col">Status</th><th scope="col">Profile</th></tr></thead><tbody>{directoryStudents.map((student) => <tr key={student.id}><td><span className="directory-student"><span className="owner-mark">{student.initials}</span><strong>{student.name}</strong></span></td><td>{student.programAffiliation}</td><td>{student.leadershipRole}</td><td>{student.primaryWorkstream}</td><td><span className={`student-active ${student.active ? "is-active" : "is-inactive"}`}>{student.active ? "Active" : "Inactive"}</span></td><td><button className="button button-secondary" type="button" onClick={() => router.push(`/students/${student.id}`)}>View profile <ArrowRight size={15} /></button></td></tr>)}</tbody></table> : <p className="empty-state">No students match these filters.</p>}</div>
    </section> : <>
