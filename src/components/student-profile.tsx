...
import { loadMentorResponses } from "@/lib/update-storage";
...
function HistoryEntry({ update, expanded, onToggle }: { update: WeeklyUpdate & { status: UpdateStatus; mentorResponse?: MentorResponse }; expanded: boolean; onToggle: () => void }) {
...
export function StudentProfile({ studentId }: { studentId: string }) {
  ...
  const history = updates.map((update) => ({ ...update, status: getDisplayStatus(update), mentorResponse: mentorResponses[update.id] ?? update.mentorResponse })).sort((a, b) => b.meetingDate.localeCompare(a.meetingDate) || b.submittedAt.localeCompare(a.submittedAt));
  ...
  return <main className="page-frame profile-page">
    ...
    <section className="profile-focus"><div><p className="eyebrow">Current focus</p><p>{student.currentFocus}</p></div><Link className="button button-primary" href="/submit">Open update form</Link></section>
